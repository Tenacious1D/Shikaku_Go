...
namespace Shikaku.UI.Buildings
{
    ...
    public sealed class AdventureCityScenery : VisualElement
    {
        ...
        private float _scale = 1, _origin;
        private bool _mirror;
        private Painter2D _p;
        private static readonly Vector2[] TreeSites = {
            new Vector2(87, 279), new Vector2(119, 259), new Vector2(92, 350),
            new Vector2(499, 448), new Vector2(565, 348), new Vector2(620, 315),
            new Vector2(706, 351), new Vector2(749, 394), new Vector2(697, 439),
            new Vector2(556, 405), new Vector2(67, 652), new Vector2(78, 723),
            new Vector2(218, 795), new Vector2(437, 842), new Vector2(738, 667),
            new Vector2(745, 720), new Vector2(529, 746)
        };

        public AdventureCityScenery()
        {
        ...
        private Vector2 Screen(Vector2 p) => new Vector2((_mirror ? 790 - p.x : p.x) * _scale, (p.y + _origin) * _scale);
        ...
        private void Draw(MeshGenerationContext context)
        {
            ...
            {
                _origin = (_districts - 1 - district) * 920;
        ...
        {
            // Shared courts hold multiple chapters; these are public paving, not foundations.
            Box(new Vector2(315, 370), 325, 175, 3, _paving);
            Box(new Vector2(253, 726), 325, 175, 3, _paving);
            Box(new Vector2(645, 678), 175, 180, 3, _paving);
            // A small public garden gives the incomplete upper-right block a purpose.
            Box(new Vector2(647, 389), 180, 182, 4, _sidewalk);
            Box(new Vector2(647, 386), 155, 158, 3, _lawn);
            Plane(new Vector2(647, 386), 158, 22, _paving);
            Plane(new Vector2(647, 386), 20, 158, _paving);
            ...
            // Parked cars sit consistently beside a curb; no runtime randomness or movement.
            ...
            Fountain(new Vector2(647, 386));
            Bench(new Vector2(607, 436));
            Bench(new Vector2(691, 355));
            Bench(new Vector2(286, 436));
            Lamp(new Vector2(321, 601));
            Lamp(new Vector2(537, 655));
            Lamp(new Vector2(675, 875));
            // Stable, bounded jitter gives each neighborhood its own planting arrangement.
            var random = new System.Random(417 + seed * 7919);
            for (int i = 0; i < TreeSites.Length; i++)
            {
                Vector2 site = TreeSites[i] + new Vector2(random.Next(-5, 6), random.Next(-4, 5));
                Tree(site, 0.78f + (float)random.NextDouble() * 0.35f, i % 4 == 0);
            }
            // Low planters and flowers add details at pedestrian scale.
        ...
        private void Car(Vector2 center, bool reverse, Color color)
        {
            ...
            Circle(center + new Vector2(-12, 0), 3.3f, new Color32(35, 46, 49, 255));
            Circle(center + new Vector2(12, 0), 3.3f, new Color32(35, 46, 49, 255));
        }
        ...
        private void Lamp(Vector2 ground)
        {
            Plane(ground, 10, 10, _sidewalk);
            Stroke(ground, ground + new Vector2(0, -45), 3, Shade(_sidewalk, 0.45f));
            Stroke(ground + new Vector2(0, -45), ground + new Vector2(10, -50), 3, _sidewalk);
            if (_night > 0.5f)
            {
                Circle(ground + new Vector2(10, -48), 19, new Color(1, 0.78f, 0.38f, 0.07f));
                Circle(ground + new Vector2(10, -48), 11, new Color(1, 0.83f, 0.47f, 0.14f));
            }
            Circle(ground + new Vector2(10, -48), 4, _night > 0.5f ? new Color32(255, 221, 147, 255) : _paving);
        }
